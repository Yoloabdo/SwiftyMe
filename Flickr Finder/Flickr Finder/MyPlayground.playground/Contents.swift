...
let API_KEY = "ede194bd408994974e00ff0eb7c8a048"
let EXTRAS = "url_m"
let SAFE_SEARCH = "1"
...
func getImageFromFlickrBySearch(methodArguments: [String : AnyObject]) {
    ...
    let task = session.dataTaskWithRequest(request) { (data, response, error) in
        ...
        let returnCount = photosDictionary.count
        if returnCount == 0 {
            //self.imageTitle.text = "No images found"
        }
        let randInt = Int(arc4random_uniform(UInt32(returnCount)))
        let randomImage = photosDictionary["photo"]
        print(randomImage)
